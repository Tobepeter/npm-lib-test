...
function test_basic() {
  console.log('\n=== 测试基础模板渲染 ===')
  const templateStr = '<h1>{{title}}</h1><ul>{{each users}}<li>{{$value}}</li>{{/each}}</ul>'
  const data = {
    title: '用户列表',
    users: ['张三', '李四', '王五'],
  }
...
function test_cond() {
  ...
  console.log('管理员视图：')
  console.log(template.render(templateStr, adminData))
  ...
  console.log('\n普通用户视图：')
  console.log(template.render(templateStr, userData))
}
...
function test_file() {
  ...
  // 创建模板文件
  const templateFile = join(templateDir, 'page.art')
  ...
  // 使用文件模板渲染
  const rendered = template(templateFile, pageData)
