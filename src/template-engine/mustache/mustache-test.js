...
function test_basic() {
  console.log('\n=== 测试基础模板渲染 ===')
  const template = '<h1>{{title}}</h1><ul>{{#users}}<li>{{.}}</li>{{/users}}</ul>'
  const data = {
    title: '用户列表',
    users: ['张三', '李四', '王五'],
  }
...
function test_cond() {
  ...
  console.log('管理员视图：')
  console.log(Mustache.render(template, adminData))
  ...
  console.log('\n普通用户视图：')
  console.log(Mustache.render(template, userData))
}
...
function test_file() {
  ...
  // 创建模板文件
  const templateFile = join(templateDir, 'page.mustache')
  ...
  const rendered = Mustache.render(templateContent, pageData)
